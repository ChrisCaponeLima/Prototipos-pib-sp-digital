...
// 2. Remove parâmetros que causam falhas no Neon Serverless / Drivers Node.js
const connectionString = rawUrl
  .replace('channel_binding=require&', '')
  .replace('&channel_binding=require', '')

if (!connectionString) {
  console.error('❌ ERRO CRÍTICO: Nenhuma string de conexão válida encontrada no .env!')
}
...
// Tratamento global de erro no pool
pool.on('error', (err) => {
